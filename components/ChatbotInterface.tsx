...
export default function ChatbotInterface() {
  ...
  const handleCommand = async (command: string) => {
    ...
    switch (cmd) {
      ...
      case "/script":
        if (args[0]) {
          await generateScript(Number.parseInt(args[0]));
        } else {
          throw new Error(
      ...
      case "/modify":
        if (args.length >= 2) {
          const scriptIndex = Number.parseInt(args[0]) - 1;
  ...
  const handleProviderChange = (
    ...
    setSelectedProvider(event.target.value as "openai" | "openrouter");
  };
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {messages.map((message, index) => (
                  <div
                    key={index}
                    className={`mb-4 ${message.message_type === "user" ? "text-right" : "text-left"}`}
                  >
                    <div
